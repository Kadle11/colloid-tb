...
pushd $ycsb_path
if [ ! -d $ycsb_path ]; then
    echo "YCSB path not found: $ycsb_path"
    exit 1
fi
sudo numactl -C $app_core_list --membind $local_numa $ycsb_path/bin/ycsb load redis -s -P $workload_path -p "redis.host=127.0.0.1"
popd
...
for bg_cores in 0 5 10 15; do
    ...
    pushd $ycsb_path
    echo "Running $config"
    $ycsb_path/bin/ycsb run redis -s -P $workload_path -p "redis.host=127.0.0.1" > $stats_path/$config.app.txt 2> $stats_path/$config.stderr.txt &
    pid_app=$!;
    all_pids+=($pid_app);
