...
latency_bm_path=$COLLOID_HOME/apps/memory_microbenchmarks/build/bin/kv_latency_benchmark
...
for bg_cores in 0 5 10 15; do
    ...
    echo "Running $config"
    numactl --membind=$local_numa -C $app_core_list $latency_bm_path --threads 16 --duration 120 --warmup 30 --dist zipf > $stats_path/$config.app.txt 2> $stats_path/$config.stderr.txt &
    pid_app=$!;
    all_pids+=($pid_app);
...
for bg_cores in 0 5 10 15; do
    ...
    # Run Latency Benchmark
    echo "Running $config"
    numactl --membind=$local_numa -C $app_core_list $latency_bm_path --threads 16 --duration 120 --warmup 30 --dist zipf --nt 1 > $stats_path/$config.app.txt 2> $stats_path/$config.stderr.txt &
    pid_app=$!;
    all_pids+=($pid_app);
