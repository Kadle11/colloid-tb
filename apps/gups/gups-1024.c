...
void *thread_function(void *arg) {
    ...
    size_t hot_slots = args->hot_size / 1024;
    size_t cold_slots = (args->buf_size)/1024;
    char *start;
    size_t slots;
    char *chunk;
    int k;
    while(count < 999999999999999ULL) {
        ...
        __m128i chx;
        chx = _mm512_extractii128_si256(sum, 0);
        ...
        chx = _mm512_extractii128_si256(sum, 1);
        chx0 = _mm_extract_epi32(chx, 0);
        chx1 = _mm_extract_epi32(chx, 1);
        chx2 = _mm_extract_epi32(chx, 2);
        chx3 = _mm_extract_epi32(chx, 3);
        read_checksum += chx0 + chx1 + chx2 + chx3;
        chx = _mm512_extractii128_si256(sum, 2);
        chx0 = _mm_extract_epi32(chx, 0);
        chx1 = _mm_extract_epi32(chx, 1);
        chx2 = _mm_extract_epi32(chx, 2);
        chx3 = _mm_extract_epi32(chx, 3);
        read_checksum += chx0 + chx1 + chx2 + chx3;
        chx = _mm512_extractii128_si256(sum, 3);
        chx0 = _mm_extract_epi32(chx, 0);
        chx1 = _mm_extract_epi32(chx, 1);
        chx2 = _mm_extract_epi32(chx, 2);
        chx3 = _mm_extract_epi32(chx, 3);
