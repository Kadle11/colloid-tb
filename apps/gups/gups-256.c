...
void *thread_function(void *arg) {
    ...
    while(count < 999999999999999ULL) {
        ...
        for(i = 0; i < 32768; i++) {
            ...
            VEC_TYPE mm_a = VEC_LOAD(&chunk[0]);
            VEC_STORE(&chunk[0], VEC_ADD(mm_a, val));
            mm_a = VEC_LOAD(&chunk[64]);
            VEC_STORE(&chunk[64], VEC_ADD(mm_a, val));
            mm_a = VEC_LOAD(&chunk[128]);
            VEC_STORE(&chunk[128], VEC_ADD(mm_a, val));
            mm_a = VEC_LOAD(&chunk[192]);
            VEC_STORE(&chunk[192], VEC_ADD(mm_a, val));
            count++;
        }
        #else
