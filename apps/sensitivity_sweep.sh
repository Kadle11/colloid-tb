...
if [ $# -lt 5 ]; then
    echo "Usage: $0 <config> <duration> <app_cores> <local_size> <workload_cmd> <workload_init_cmd>"
    exit 1
fi
...
# If Workload Init Command is provided, run it
if [ -n "$workload_init_cmd" ]; then
    echo "Running workload init command: $workload_init_cmd"
    numactl -C $app_core_list --membind=$local_numa $workload_init_cmd
    sleep 5
fi

for bg_cores in "${cross_numa_interference_sweep[@]}"; do

    for llc in "${llc_sweep[@]}"; do
        config=$1-app$app_cores-bg$bg_cores-llc$llc

        echo "Config: $config, LLC: $llc, BG_CORES: $bg_cores"
        all_pids=()
        ...
        if [ $bg_cores -gt 0 ]; then
            ...
            PYTHONPATH=$PYTHONPATH:$mio_path python3 -m mio cn-traffic-mio --ant_cpus $bg_core_list --ant_num_cores $bg_cores --ant_mem_numa 0 --ant stream --ant_inst_size 64 --ant_duration 1000000 "${mio_opts[@]}" &
            pid_mio=$!;
            all_pids+=($pid_mio);
            sleep 7;
        fi

        pqos -a "core:$llc=0-143"

        echo "Local mem size"
        echo $(numastat -m | grep MemFree)
        echo $(numastat -m | grep MemFree) > $stats_path/$config.memfree.txt

        mlc --bandwidth_matrix > $stats_path/$config.mlc.txt
